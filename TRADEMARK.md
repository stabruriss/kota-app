# Trademark Policy

"Kota" is a trademark of Nan Wu.

This policy is intended to let people accurately refer to Kota while preventing
confusing or misleading uses of the Kota name, logo, and official identity.

## What This Policy Covers

This policy covers the "Kota" word mark, the Kota logos, and the official Kota
visual identity. These are not licensed under the Apache License, Version 2.0.

Within Kota, certain feature and concept names — such as Tavern, Dreams, Source
Hero, Incarnation, Violet, Ember, Bartender, Magi, Laughing Man, BBS, and
Puppeteer — refer to specific Kota features. These names, together with the
official UI/UX designs, copy, artwork, and brand expression that identify each
feature, are part of Kota's official product identity and are not licensed
under Apache-2.0 (see `LICENSE-ASSETS`).

You may not use these feature names, or their corresponding official designs
and brand expression, in a way that implies your product, service, package,
fork, or distribution is an official Kota feature, or that it is endorsed by,
sponsored by, or affiliated with Kota.

This policy does not restrict functional use of these names as identifiers
inside source code that is licensed under Apache-2.0.

## Allowed Uses

You may:

- refer to the Kota project by name;
- say that software works with Kota or is compatible with Kota, if true;
- link to the official Kota website or repository;
- make truthful, non-misleading statements about the origin of the source code;
- reproduce required notices from the Apache-2.0 `NOTICE` file.

## Not Allowed

You may not, without permission:

- use "Kota" as the name of an unofficial product, fork, service, package, or
  app, or use a confusingly similar name, package name, app name, domain, logo,
  or visual identity;
- use Kota logos, icons, official visual assets, or official brand identity;
- use Kota official UI/UX, copy, artwork, or brand expression in a way that
  suggests official status, endorsement, sponsorship, or affiliation;
- imply that an unofficial fork, distribution, service, or product is the
  official Kota distribution.

## Forks And Distributions

Forks, modified distributions, hosted services, and packages based on Kota must
use a different name and must not imply they are the official Kota distribution.

Descriptive phrases such as "based on Kota" or "compatible with Kota" are
allowed when they are truthful and do not imply official status, endorsement,
sponsorship, or affiliation.

## Contact

For non-security brand-use questions, open an issue in this repository. Do not
include confidential information in public issues.
